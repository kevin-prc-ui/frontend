...
const ChatComponent = ({
  ...
  const validateFile = (file) => {
    if (!ALLOWED_TYPES.includes(file.type)) {
      return `Archivo no soportado: ${file.name} (Tipo: ${
        file.type || "desconocido"
      }). Permitidos: PNG, JPG, PDF.`;
    }
  ...
  // --- Render Logic ---
  return (
    <div className="flex-1 flex flex-col h-[600px] lg:h-full max-h-[80vh] bg-white rounded-lg shadow-lg overflow-hidden border border-gray-200">
      {" "}
      {/* Altura ajustada */}
      {/* Header */}
        ...
        {isLoadingMessages ? (
          ...
          messages.map((msg) => {
            ...
            const senderName =
              msg.sender?.nombre ||
              msg.sender?.nombres ||
              "Usuario Desconocido"; // Verificar 'nombre' y 'nombres'
            ...
            if (
              ...
              console.warn(
                "Saltando renderizado de mensaje con ID o timestamp faltante:",
                msg
              );
                  ...
                  {msg.content && (
                    <p
                      className={`text-sm whitespace-pre-wrap break-words ${
                        msg.messageType !== "TEXT" ? "italic text-gray-200" : ""
                      }`}
                    >
                      {msg.content}
                    </p>
            ...
            onKeyDown={(e) => {
              ...
              if (e.key === "Enter" && !e.shiftKey) {
                ...
                if (!isSendButtonDisabled) {
                  // Solo enviar si el botón no está deshabilitado
