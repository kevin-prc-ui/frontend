...
import { createMotivo } from "../../services/MotivoService"; // Assuming this service function exists

export const AddMotivo = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      await createMotivo({ nombre });
      toast.success("Motivo creado correctamente");
      navigate("/admin/helpdesk/motivos"); // Navigating to /motivoss as per existing pattern
    } catch (err) {
      console.error("Error creating motivo:", err);
      const errorMessage = err.response?.data?.message || "Error al crear el motivo. Por favor, inténtelo de nuevo.";
        ...
        <div className="bg-white shadow-md rounded-lg p-4 max-w-lg mx-auto">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="flex items-center justify-end space-x-3 mt-6">
              <button
                type="button"
                onClick={() => navigate("/admin/helpdesk/motivos")} // Navigating to /motivoss
